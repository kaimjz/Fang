...
Nature.Page.QuickPagerDefaultInfo = {
    ...
    //分页控件模板的路径和文件名
    templetPath: "/Theme/js/easyPager/template.htm",
    //分页控件css文件的路径和文件名
    cssPath: "/Theme/js/easyPager/EasyPager.css",
...
Nature.Page.QuickPagerExtend = {
    CreateNavi: function (pageInfo) {
        ...
        if ((parseInt(pageInfo.thisPageIndex) - naviCount2) < 1) {
            s1 = 1; s2 = pageInfo.naviCount;
        } else if ((parseInt(pageInfo.thisPageIndex) + naviCount2) >= pageInfo.pageCount) {
            s1 = pageInfo.pageCount - pageInfo.naviCount + 1;
            s2 = pageInfo.pageCount;
            if (s1 < 1) s1 = 1;
        } else {
            s1 = parseInt(pageInfo.thisPageIndex) - naviCount2;
            s2 = parseInt(pageInfo.thisPageIndex) + naviCount2;
        ...
        if (s2 - s1 > 8) {
            for (i = s1; i <= s1 + 8; i++) { //当前页
                if (i == pageInfo.thisPageIndex) {
                    var nnow = "<a class=\"navi\" style=\"border-top: 3px solid #578d83;\">{text}</a>";
                    temp = nnow.replace("{text}", "<font style=\"color:#999\">" + i + "</font>");
                } else {
...
//绑定列表
function GetPageList(url, para, contentid, pageid, pagesize, obj) {
 //   alert($.cookie('url') + "\\" + $.cookie('para') + "||" + $.cookie('contentid') + "||" + $.cookie('thisPageIndex'));
    try {

        if ($.cookie("useCookie") == "true") {
            url = $.cookie('url');
            para = $.cookie('para');
            contentid = $.cookie('contentid');
            pageid = $.cookie('pageid');
            pagesize = $.cookie('pageSize');
            if ($.cookie('thisPageIndex') != null) {

                pageIndex = $.cookie('thisPageIndex');
            } else {
                pageIndex = 1;
            }
    ...
    $.post(url + para, {
        ...
        setpage(url, para, contentid, pageid, obj, count, pageSize, pageIndex);
...
function setpage(url, para, contentid, pageid, obj, count, pageSize, thisPageIndex) {
    ...
    page.pagerInfo = {
        ...
        OnPageChange: function (oldPageIndex, thisPageIndex) {
            ...
            $.post(url + para, {
                ...
                result.thisPageIndex = thisPageIndex;

                $.cookie('thisPageIndex', thisPageIndex); //Cookie记录pageIndex  
